...
function M.vim_opt_toggle(opt, on, off, name)
  ...
  require("astronvim").notify(name .. " " .. (is_off and "Enabled" or "Disabled"))
end
...
function M.better_search(key)
  return function()
    ...
    if not searched and type(error) == "string" then require("astronvim").notify(error, vim.log.levels.ERROR) end
  end
end
